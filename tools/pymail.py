...
mail_user="akaisorani@126.com"
mail_pass="HANSMTP126"
	...
	try:
		...
		for index in range(mail_amount,0,-1):
			...
			if (From==None or (From in msgFrom)) and (Subject==None or (Subject in msgSubject)):
				...
				msg_content=b'\r\n'.join(lines).decode('utf-8')
				ret_msg=Parser().parsestr(msg_content)
...
if __name__=="__main__":
	...
	if send_mail(test_to_list,"测试邮件","Welcome to <a href='http://akaisora.tech'>Akaisora's blog</a>","test.png"):
		print("发送成功")
	else:
		print("发送失败")
